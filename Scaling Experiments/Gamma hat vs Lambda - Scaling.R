...
sim.output.scaled = pblapply(seq_len(K), function(i) {
  ...
  ############################################
  ### Choose lambda at each value of gamma ###
  ############################################
...
data.sim.cv.scaled.raw = c()


#Convert simulation output to a data frame
for (i in seq_along(sim.output.scaled)) {
  this.info = sim.output.scaled[[i]]
  ...
  data.sim.scaled = rbind(data.sim.scaled, this.data)
  ...
  data.sim.cv.scaled.raw = rbind(data.sim.cv.scaled.raw, this.data.cv)
}

### Convert CV data from wide to tall
data.sim.cv.scaled = data.sim.cv.scaled.raw %>%
...
##################################################
plot.gamma.hat = ggplot(data.sim.scaled,
                        aes(
                          x = log(lambda.prop),
                          y = gamma,
                          group = iteration,
...
#   )
#   data.lambda.hat.scaled = rbind(data.lambda.hat.scaled, this.data)
# }

mean.lambda.hats.scaled = data.sim.cv.scaled %>%
  group_by(gamma, type) %>%
  summarise(ave = mean(lambda))
mean.lambda.hats.scaled = data.frame(mean.lambda.hats.scaled)
...
lambda.gamma.0.scaled = filter(data.sim.cv.scaled, gamma == gamma.0)
lambda.gamma.0.scaled$gamma.hat = 0
for (i in seq_len(K)) {
  print(paste0(i, " of ", K))
  this.lambdas = filter(lambda.gamma.0.scaled, iteration == i)
  ...
  for (j in seq_along(these.lambdas)) {
    ...
    lambda.gamma.0.scaled$gamma.hat[ind.data] = gamma.hat
  }
}

plot.gamma.0 = ggplot(lambda.gamma.0.scaled,
