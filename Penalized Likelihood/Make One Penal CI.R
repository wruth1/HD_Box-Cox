...
all.intervals = pblapply(seq_len(M), function(i) {
  ...
  opt.lik = optimize(pen.lik.CV.lasso, c(gamma.min, gamma.max),
  ...
  ints = lapply(threshs, function(thresh) {
    ...
    if(lik.left > thresh) {
      ...
      a = uniroot(pen.lik.root.CV.lasso, c(gamma.min, gamma.hat),
    ...
    if(lik.right > thresh) {
      ...
      b = uniroot(pen.lik.root.CV.lasso, c(gamma.hat, gamma.max),
                  X = X, Y = Y, val = thresh, folds=folds, 
                  all.lambdas = all.lambdas)$root
    }
    return(c(a,b))
...
write.table(results, "Penalized Likelihood/Coverages - LASSO.csv",
